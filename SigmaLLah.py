...
def obj_fill():
    ...
    try:
        ...
        for tri in triangles:
            ...
            p1 = np.array(tri[0], dtype=float)
            p2 = np.array(tri[1], dtype=float)
            p3 = np.array(tri[2], dtype=float)

            edge1 = p2 - p1
            edge2 = p3 - p1

            faceN = np.cross(edge1, edge2)
            fNL = np.linalg.norm(faceN)
            if fNL < 1e-6:
                continue
            faceN = faceN / fNL

            # Самое длинное ребро как основание
            edges = [
                (p1, p2, p3),
                (p2, p3, p1),
                (p3, p1, p2),
            ]
            best = max(edges, key=lambda e: np.linalg.norm(e[1] - e[0]))
            a, b, apex = best
            ...
            if edgeLen < 1e-6:
                continue
            axisX = edgeVec / edgeLen

            # axisY перпендикулярен axisX в плоскости грани
            axisY = np.cross(faceN, axisX)
            aYL = np.linalg.norm(axisY)
            if aYL < 1e-6:
                continue
            axisY = axisY / aYL
            ...
            # Горизонтальный сдвиг центра
            # (треугольник может быть косым — центр по X смещён)
            toApex_x = float(np.dot(toApex, axisX))
            
            # Центр блока
            blockCenter = midEdge + axisY * (h * 0.5) + axisX * (toApex_x * 0.5)

            # Ширина блока по X с учётом скоса
            # Проекции всех вершин на axisX
            proj_x = [
                float(np.dot(p1 - blockCenter, axisX)),
                float(np.dot(p2 - blockCenter, axisX)),
                float(np.dot(p3 - blockCenter, axisX)),
            ]
            ...
            blocks.append({
                "pos":   blockCenter.tolist(),
                "axisX": axisX.tolist(),
                "axisY": axisY.tolist(),
                "axisZ": axisZ.tolist(),
                "size":  [float(width), float(h), 0.05],
            })

        print(f"[obj] {len(triangles)} tri -> {len(blocks)} blocks")
        return jsonify({"ok": True, "blocks": blocks})

    except Exception as e:
        print(f"[obj] ERROR: {traceback.format_exc()}")
        return jsonify({"ok": False, "error": str(e)}), 500
